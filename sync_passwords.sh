...
function sync_passwords ()
{
	...
	local human_readable_remote_mtime=`get_remote_passwords_mtime 2>/dev/null`
    ...
    local local_mtime_in_seconds_since_epoch=$(date -d "$human_readable_local_mtime" +%s)
    local remote_mtime_in_seconds_since_epoch=$(date -d "$human_readable_remote_mtime" +%s)
